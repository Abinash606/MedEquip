    ...
    <div class="glass-card">
        ...
        <div id="calendarView" class="schedule-view px-3 pb-3">
            <div class="d-flex align-items-center gap-3 py-2 mb-2 flex-wrap">
                ...
                <span class="badge px-2 py-1" style="background:#10b981;"><i class="fa-solid fa-clipboard-check me-1"></i>Inspection</span>
                <span class="badge px-2 py-1" style="background:#8b5cf6;"><i class="fa-solid fa-calendar-plus me-1"></i>Next Due</span>
                <span class="badge px-2 py-1" style="background:#f97316;"><i class="fa-solid fa-triangle-exclamation me-1"></i>Overdue</span>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr class="sched-row" data-status="<?= esc(strtolower($wo['status'] ?? '')) ?>" style="cursor:pointer;"
                                    ...
                                    <td>
                                        <div><?= esc($wo['site_name'] ?? '—') ?></div><small class="text-muted"><?= esc($wo['customer_name'] ?? '') ?></small>
                                    </td>
                                    <td><?= esc($equip) ?></td>
                            ...
                            <?php endforeach; ?>
                            <?php if (empty($workOrders)): ?><tr>
                                    <td colspan="8" class="text-center text-muted py-3">No work orders assigned to you.</td>
                                </tr><?php endif; ?>
                        </tbody>
                    </table>
                </div>
                    ...
                    <table id="schedTableInsp" class="table service-table align-middle" style="width:100%">
                        ...
                        <tbody>
                            <?php foreach ($inspections as $insp):
                                ...
                                $sc = in_array($sl, ['closed/complete','closed_complete','closed','complete','completed','pass']) ? 'bg-success' : ($sl === 'fail' ? 'bg-danger' : ($sl === 'repair' ? 'bg-warning text-dark' : 'bg-secondary'));
                                $isDone = in_array($sl, ['completed', 'complete', 'closed', 'closed/complete', 'closed_complete']);
                            ...
                            ?>
                                <tr class="sched-row" data-status="<?= esc($sl) ?>" style="cursor:pointer;"
                                    ...
                                    <td><span class="badge <?= $sc ?>"><?= esc($insp['status'] ?? 'In Progress') ?></span></td>
                            ...
                            <?php endforeach; ?>
                            <?php if (empty($inspections)): ?><tr>
                                    <td colspan="7" class="text-center text-muted py-3">No inspections assigned to you.</td>
                                </tr><?php endif; ?>
                        </tbody>
                    </table>
                </div>
...
<script>
    ...
    function applySchedFilter() {
        ...
        if (!_schedShowCompleted) {
            $.fn.dataTable.ext.search.push(function(settings, data, idx) {
                ...
                var rowApi = (tid === 'schedTableWO' ? _dtWO : _dtInsp);
                var row = rowApi ? rowApi.row(idx).node() : null;
                var st = row ? ($(row).data('status') || '') : '';
                return !_schedIsCompletedStatus(tid, st);
            });
        }
        if (_dtWO) _dtWO.draw();
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (typeof FullCalendar === 'undefined') {
            calendarEl.innerHTML = '<div class="alert alert-warning m-3">FullCalendar failed to load.</div>';
            return;
        }
        ...
        window._fcCalendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: function(fetchInfo, successCallback, failureCallback) {
                fetch('<?= site_url('technician/scheduling/events') ?>?show_completed=' + (_schedShowCompleted ? '1' : '0'))
            ...
            eventClick: function(info) {
                ...
                if (type === 'next_due') {
                    // Show a simplified next-due modal (tech can't send reminder)
                    var daysOut = p.days_until_due || 0;
                    ...
                    var body = '<table class="table table-sm table-borderless text-white">'
                        + '<tr><th class="text-muted pe-3">Customer</th><td style="color:#a78bfa;font-weight:bold;">' + (p.customer_name || '') + '</td></tr>'
                        + '<tr><th class="text-muted pe-3">Site</th><td>' + (p.site_name || '') + '</td></tr>'
                        + '<tr><th class="text-muted pe-3">Next Due</th><td>' + (info.event.startStr || '') + '</td></tr>'
                        + '<tr><th class="text-muted pe-3">Status</th><td>' + urgency + '</td></tr>'
                        + '</table>';
                    document.getElementById('schedItemModalTitle').innerHTML =
                        '<i class="fa-solid fa-calendar-plus me-2" style="color:#8b5cf6;"></i>Next Due Inspection';
                    document.getElementById('schedItemModalBody').innerHTML = body;
                    var m = document.getElementById('schedItemModal');
                    if (m) new bootstrap.Modal(m).show();
                } else {
                    showSchedModal(
                        type === 'inspection' ? 'insp' : 'wo',
                        info.event.id || '', info.event.title || '',
                        p.status || '', p.priority || '',
                        '', p.site_name || '', p.customer_name || '',
                        '', info.event.startStr || '', p.next_due_date || '');
                }
            },
        ...
        var tg1 = document.getElementById('toggleCompleted');
        if (tg1) tg1.addEventListener('change', function() { syncToggles(this.checked); });
        var tg2 = document.getElementById('schedShowCompleted');
        if (tg2) tg2.addEventListener('change', function() { syncToggles(this.checked); });
    });
</script>
