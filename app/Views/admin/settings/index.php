                        ...
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div>
                                ...
                                <small class="text-muted">Dynamically populated from Equipment DB. Toggle EST &amp; CAL
                                    flags per device row. New equipment added via Sites appears here after
                                    refresh.</small>
                            </div>
                            <button class="btn btn-outline-secondary btn-sm" id="btnRefreshEquipment"
                                title="Reload equipment list from database">
            ...
            <form id="iqNoteForm">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" id="iq-note-title" class="form-control"
                            placeholder="Enter note title..." required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Description</label>
                        <textarea name="note" id="iq-note-text" class="form-control" rows="4"
...
$(function() {
    ...
    if (hash) {
        setTimeout(() => activateTab(hash), 100); // ← wrap in setTimeout
    } else {
    ...
    function activateTab(targetId) {
        ...
        if (targetId === 'equipmentSettings' && typeof equipmentTable !== 'undefined') {
            equipmentTable.columns.adjust().draw(); // ← ADD THIS
        ...
        if (targetId === 'iqNotesSettings' && typeof iqTable !== 'undefined') {
            iqTable.columns.adjust().draw(); // ← ADD THIS
    ...
    const iqTable = $('#iqNotesTable').DataTable({
        ...
        columns: [{
            ...
            },
            {
                data: 'note',
                render: function(d) {
                    return d ? d.substring(0, 80) + (d.length > 80 ? '...' : '') : ''
                }
            },
            {
                data: null,
