...
<div class="glass-card">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom p-3 flex-wrap gap-2" style="border-color:rgba(255,255,255,.08)!important;">
        <div class="d-flex gap-2 align-items-center">
            <button class="btn btn-light btn-sm" id="btnPrev" onclick="shiftDate(-1)"><i class="fa-solid fa-chevron-left"></i></button>
            <span class="fw-bold mx-2" id="currentDateLabel">Today, <?= date('M j, Y') ?></span>
            <button class="btn btn-light btn-sm" id="btnNext" onclick="shiftDate(1)"><i class="fa-solid fa-chevron-right"></i></button>
        </div>
        ...
        <ul class="nav nav-tabs border-0 gap-1" id="schedTabs" role="tablist">
            ...
            <li class="nav-item">
                <button class="nav-link fw-semibold px-3 py-1" data-bs-toggle="tab" data-bs-target="#tabInsp" type="button">
                    ...
                    <span class="badge bg-secondary ms-1" id="inspTabCount"><?= count($scheduledInspections ?? []) ?></span>
                </button>
            </li>
        </ul>
        ...
        <div class="tab-pane fade show active" id="tabWO">
            <div class="table-responsive">
                <table id="schedTableWO" class="table service-table align-middle" style="width:100%">
                    <thead>
                        <tr>
                            <th>#</th><th>Title</th><th>Site / Customer</th><th>Equipment</th>
                            <th>Technician</th><th>Priority</th><th>Status</th><th>Start Date</th><th>End Date</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($upcomingWO as $wo):
                            $priClass = match(strtolower($wo['priority'] ?? '')) {
                                'critical'=>'bg-danger','high'=>'bg-warning','medium'=>'bg-info','low'=>'bg-success',default=>'bg-secondary'
                            };
                            $stClass = match(strtolower($wo['status'] ?? '')) {
                                'closed','completed'=>'bg-success','in_progress'=>'bg-warning','open'=>'bg-primary',default=>'bg-secondary'
                            };
                        ...
                        <tr class="sched-row" data-status="<?= esc(strtolower($wo['status'] ?? '')) ?>"
                            ...
                            <td><div><?= esc($wo['site_name'] ?? '—') ?></div><div class="text-muted small"><?= esc($wo['customer_name'] ?? '') ?></div></td>
                            <td><?= esc($equip) ?></td>
        ...
        <div class="tab-pane fade" id="tabInsp">
            <div class="table-responsive">
                <table id="schedTableInsp" class="table service-table align-middle" style="width:100%">
                    <thead>
                        <tr>
                            <th>Inspection ID</th><th>Type</th><th>Site</th><th>Customer</th>
                            <th>Technician</th><th>Status</th><th>Scheduled</th><th>Next Due</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($scheduledInspections ?? [] as $insp):
                            $inspId = 'INSP-' . date('Ymd', strtotime($insp['scheduled_at'] ?? 'now')) . '-' . strtoupper(substr(md5($insp['group_id'] ?? $insp['id']), 0, 6));
                            $stLow  = strtolower($insp['status'] ?? '');
                            $stCls  = $stLow === 'pass' ? 'bg-success' : ($stLow === 'fail' ? 'bg-danger' : 'bg-warning text-dark');
                        ?>
                        <tr class="sched-row" data-status="<?= in_array($stLow,['pass','fail','repair','completed']) ? 'closed' : 'open' ?>"
                            style="cursor:pointer;"
                            ...
                            <td><span class="badge <?= $stCls ?>"><?= esc(ucfirst($insp['status'] ?? 'In Progress')) ?></span></td>
                        ...
                        <tr><td colspan="8" class="text-center text-muted py-4">No inspections scheduled.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
...
function _fmtDate(d) {
    var opts = { weekday: undefined, year: 'numeric', month: 'short', day: 'numeric' };
...
function showSchedModal(type, id, title, status, priority, tech, site, customer, equipment, date1, date2) {
    ...
    var rows = '<tr><th class="text-muted pe-3" style="white-space:nowrap;">Type</th><td>' + badge + '</td></tr>'
        + '<tr><th class="text-muted pe-3">' + (isWO ? 'WO #' : 'ID') + '</th><td class="fw-semibold">' + id + '</td></tr>'
        + '<tr><th class="text-muted pe-3">Title</th><td>' + title + '</td></tr>'
        + '<tr><th class="text-muted pe-3">Status</th><td><span class="badge ' + stCls + '">' + status + '</span></td></tr>'
        ...
        + (date1 ? '<tr><th class="text-muted pe-3">' + (isWO ? 'Start' : 'Scheduled') + '</th><td>' + date1.substring(0,10) + '</td></tr>' : '')
        + (date2 ? '<tr><th class="text-muted pe-3">' + (isWO ? 'End' : 'Next Due') + '</th><td>' + date2.substring(0,10) + '</td></tr>' : '');
    document.getElementById('schedItemModalTitle').textContent = title || 'Item Details';
    document.getElementById('schedItemModalBody').innerHTML = '<table class="table table-sm table-borderless mb-0">' + rows + '</table>';
...
$(function() {
    _dtWO = $('#schedTableWO').DataTable({ pageLength: 15, order: [[7, 'asc']] });
    _dtInsp = $('#schedTableInsp').DataTable({ pageLength: 15, order: [[6, 'asc']] });
    applySchedFilter();
