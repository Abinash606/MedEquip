...
function validateTechPhone(input) {
    ...
    if (!PHONE_REGEX.test(input.value.trim())) {
        input.classList.add('is-invalid');
        feedback.textContent = 'Please enter a valid phone number (digits, spaces, dashes, or parentheses only).';
        return false;
    }
...
function validateTechForm() {
    ...
    if (!valid) {
        const firstBad = document.querySelector('#technicianForm .is-invalid');
        if (firstBad) {
